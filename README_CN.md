<p align="center">
  <a href="README.md">English</a> | <strong>中文</strong>
</p>

<p align="center">
  <img src="app-icon.png" width="100" height="100" alt="磁力古哥 - 免费聚合磁力搜索引擎 Android App">
</p>

<h1 align="center">磁力古哥 Magnet Googo &mdash; 聚合磁力搜索</h1>

<p align="center">
  <strong>搜全网磁力，上磁力古哥</strong><br>
  <em>免费 &middot; 无广告 &middot; 多源聚合磁力搜索</em>
</p>

<p align="center">
  <a href="https://github.com/734496335/magnetgoogo/releases/latest">
    <img src="https://img.shields.io/github/v/release/734496335/magnetgoogo?label=%E6%9C%80%E6%96%B0%E7%89%88%E6%9C%AC&color=4285F4" alt="Latest Release">
  </a>
  <a href="https://github.com/734496335/magnetgoogo/releases">
    <img src="https://img.shields.io/github/downloads/734496335/magnetgoogo/total?color=22c55e&label=%E4%B8%8B%E8%BD%BD%E9%87%8F" alt="Total Downloads">
  </a>
  <img src="https://img.shields.io/badge/Android-7.0%2B-34A853" alt="Android 7.0+">
  <img src="https://img.shields.io/badge/%E4%BB%B7%E6%A0%BC-%E5%85%8D%E8%B4%B9-7c3aed" alt="免费">
</p>

<p align="center">
  <a href="https://magnetgoogo.com">官网</a> &nbsp;&middot;&nbsp;
  <a href="https://github.com/734496335/magnetgoogo/releases/latest">下载 APK</a> &nbsp;&middot;&nbsp;
  <a href="https://wwbdy.lanzoue.com/ighZS3pb0h0h">蓝奏云下载</a> &nbsp;&middot;&nbsp;
  <a href="https://naoshiquan.com/blog/">技术博客</a> &nbsp;&middot;&nbsp;
  <a href="https://github.com/734496335/magnetgoogo/issues">反馈</a>
</p>

---

## 磁力古哥是什么？

**磁力古哥 (Magnet Googo)** 是一款免费的 Android 磁力搜索聚合 App。它同时搜索全网多个磁力链接源，把电影、动漫、游戏、软件、Linux 镜像、纪录片等资源统一展示。不用再逐个翻找磁力站，一次搜索全部直达。

**核心优势：** 一次搜索多个独立公开索引来源，无广告、无需注册账号。

可用来源会随时间变化。当前可核验信息请以官方 [状态中心](https://magnetgoogo.com/status/)、[方法说明](https://magnetgoogo.com/methodology/) 和 [公开报告](https://magnetgoogo.com/reports/) 为准。

## 为什么做这个

平时找 Linux ISO、纪录片、开源资源时，要在十几个磁力索引站之间来回切——有的今天打不开，有的搜不到，有的弹窗广告满天飞。所以做了一个聚合器：输入一次，多源直达，挂掉的源自动跳过。

## 应用截图

<p align="center">
  <img src="screenshot-home.png" width="240" alt="磁力古哥首页 - 磁力搜索界面">
  &nbsp;&nbsp;&nbsp;
  <img src="screenshot-results.png" width="240" alt="磁力古哥搜索结果 - 聚合磁力链接">
</p>

## 功能亮点

- **多源聚合搜索** &mdash; 一次并行查询多个独立磁力 / Torrent 公共索引
- **纯净无广告** &mdash; 无广告、无弹窗、无需注册账号
- **一键复制** &mdash; 磁力链接一键复制，并可交给你使用的 BT 客户端打开
- **隐私优先** &mdash; 搜索关键词不进入 Magnet Googo 的分析埋点；仅使用匿名运营统计改善可靠性
- **来源冗余** &mdash; 持续观测来源健康度，避免把搜索体验绑定在单一站点上

## 下载安装

| 渠道 | 链接 | 说明 |
|------|------|------|
| **GitHub Releases** | [下载最新版 APK](https://github.com/734496335/magnetgoogo/releases/latest) | 始终最新 |
| **直接下载** | [快速下载](https://cn.magnetgoogo.com/download/magnetgoogo.apk) | CDN 加速 |
| **蓝奏云** | [备用下载](https://wwbdy.lanzoue.com/ighZS3pb0h0h) | 国内用户推荐（密码: 8888） |
| **官网** | [magnetgoogo.com](https://magnetgoogo.com) | 截图和详细介绍 |

> **系统要求**：Android 7.0 及以上

## 为什么选择磁力古哥？

| 对比项 | 磁力古哥 | 单个磁力站 | 浏览器搜索 |
|--------|:---:|:---:|:---:|
| 同时搜索多个独立索引 | 是 | 否 | 看情况 |
| 无广告 | 是 | 看情况 | 看情况 |
| 一键复制磁力链接 | 是 | 看情况 | 看情况 |
| 无需注册账号 | 是 | 看情况 | 看情况 |
| 提供当前来源健康证据 | [状态中心](https://magnetgoogo.com/status/) | 通常没有 | 否 |

## 可核验的项目事实

- **标准名称：** Magnet Googo；中文名称：**磁力古哥**
- **平台：** Android 7.0+（API 24+）
- **产品类型：** 聚合磁力 / Torrent 搜索工具，**不是** BT 下载客户端
- **内容边界：** Magnet Googo 不托管、不分发搜索结果所代表的资源文件
- **账号模式：** 不要求注册或登录账号
- **证据入口：** [关于](https://magnetgoogo.com/about) · [方法说明](https://magnetgoogo.com/methodology/) · [状态中心](https://magnetgoogo.com/status/) · [公开报告](https://magnetgoogo.com/reports/)

## 隐私与安全

- 搜索关键词由 App 请求第三方公开索引，不进入 Magnet Googo 的匿名分析埋点
- 会采集匿名运营统计，用于衡量可靠性和产品质量，但不绑定用户账号
- 搜索流程不需要通讯录、摄像头或定位权限
- 收藏和搜索历史保存在设备本地
- 当前政策以官网 [隐私政策](https://magnetgoogo.com/privacy) 为准

## 反馈与建议

欢迎在 [Issues](https://github.com/734496335/magnetgoogo/issues) 中提交 Bug 报告或功能建议，每条我们都会看。

## 相关阅读

更多技术背景和长文说明可参考 [naoshiquan.com](https://naoshiquan.com/blog/)：

- [2026 年最好用的 5 个磁力搜索工具](https://naoshiquan.com/blog/cili-search-tools-2026)
- [磁力链接是什么？小白完全指南](https://naoshiquan.com/blog/what-is-magnet-link)
- [磁力链接协议解析：从 BEP-9 到 DHT 网络](https://naoshiquan.com/blog/magnet-link-protocol-explained)
- [磁力猫打不开了？2026 年还能用的替代方案](https://naoshiquan.com/blog/cilimao-not-working)

## 免责声明

磁力古哥是一款搜索工具，不存储、不传播任何资源内容。搜索结果来自公开互联网，使用者应遵守当地法律法规。开发者不对搜索结果内容承担任何责任。

---

<p align="center">
  <a href="https://magnetgoogo.com">magnetgoogo.com</a><br>
  &copy; 2026 磁力古哥 Magnet Googo
</p>