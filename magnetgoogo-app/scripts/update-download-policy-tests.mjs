...
const siteConfig = JSON.parse(fs.readFileSync(path.join(repoRoot, 'magnetgoogo-site', 'config.json'), 'utf8'));
const siteDownloads = JSON.parse(fs.readFileSync(path.join(repoRoot, 'magnetgoogo-site', 'site-config.json'), 'utf8'));

const gatewayUrl = 'https://api.naoshiquan.com/download/v0.2.2/magnetgoogo-v0.2.2.apk';
const lanzouUrl = 'https://wwbdy.lanzn.com/imCPX3zgpbkb';
const githubUrl = 'https://github.com/734496335/magnetgoogo/releases/download/v0.2.2/magnetgoogo-v0.2.2.apk';

for (const config of [mgConfig, siteConfig]) {
