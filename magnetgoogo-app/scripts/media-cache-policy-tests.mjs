...
assert.match(cacheSource, /media-app-feed-cache\/3/);
assert.match(cacheSource, /media-app-detail-cache\/3/);
assert.match(cacheSource, /value\.schema_version !== 'media-app-feed-cache\/2'/);
assert.match(cacheSource, /cachedMediaFeedNeedsConsumerRefresh/);
assert.match(cacheSource, /envelope\.detail_hash !== currentItem\.remote_detail_hash/);
assert.match(cacheSource, /writeDetailEnvelope/);
assert.match(cacheSource, /rotten_tomatoes_rating: cachedItem\.rotten_tomatoes_rating \?\? currentItem\.rotten_tomatoes_rating/);
assert.match(cacheSource, /bangumi_rating: cachedItem\.bangumi_rating \?\? currentItem\.bangumi_rating/);
...
assert.match(clientSource, /feed_consumer_schema_refresh/);
assert.match(clientSource, /pointerState === 'same' && !needsConsumerRefresh/);
