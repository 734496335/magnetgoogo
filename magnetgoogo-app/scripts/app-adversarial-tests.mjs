...
await test('U3', 'movie and regional series channels form one lightweight discovery experience', () => {
  ...
  assert.match(ratings, /ROTTEN_TOMATOES_FEATURED_THRESHOLD = 60/);
  assert.match(ratings, /ROTTEN_TOMATOES_HIGH_THRESHOLD = 80/);
  assert.match(ratings, /source: '烂番茄'/);
  assert.match(ratings, /source: 'Bangumi'/);
  assert.match(ratings, /value > 0 && value <= maximum/);
...
await test('U5', 'movie and series bundles are offline-first and exclude the legacy adult feed', () => {
  ...
  assert.match(plugin, /\$\{kind\}_app_bundle/);
  assert.match(plugin, /MOVIE_APP_BUNDLE_PATH/);
  assert.match(plugin, /SERIES_APP_BUNDLE_PATH/);
  assert.match(plugin, /readAndValidateMediaBundle/);
  assert.match(plugin, /cover_content_hash/);
  assert.match(plugin, /cross-season resource/);
  assert.match(plugin, /ALLOW_EMPTY_RESOURCE_FEED/);
  assert.match(plugin, /required \$\{kind\} offline bundle is missing/);
  assert.doesNotMatch(plugin, /normalizeSeriesFeed/);
  assert.doesNotMatch(plugin, /remote covers use App cache until local cover assets are supplied/);
