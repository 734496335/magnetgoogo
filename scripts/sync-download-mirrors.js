...
const primary = String(config.download_url || '').trim();
...
if (!primary) throw new Error('site-config.json is missing the primary download URL');
...
for (const file of htmlFiles) {
  ...
  let updated = original
    .replace(/https:\/\/api\.naoshiquan\.com\/download\/v[^"'<>/]+\/magnetgoogo-v[^"'<>/]+\.apk/g, primary)
