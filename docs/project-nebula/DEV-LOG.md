...
- Production source-sync remains healthy on full pack SHA `9dd9c886...`, 357 rules /148 GREEN. mg-data shows real bot renewals through Aug13. Final unattended-risk review pushed `f9c5105`, widening the production every-8h refresh threshold from <=24h to <=32h while preserving 72h validity; renewal tests remain 5/5. The next refresh window is around Aug16 00:17 and this session has not yet observed that future cycle, so no false renewal claim is recorded.
...
- Production latest image was updated by offline code-layer overlays only, retaining rollback tags; final image `sha256:0f97daba31f83b58303fc6f52955617605f55380ffbed688f6400495821f84f1` passed host/image SHA equality and Python compile smoke.
- Real production `audit` mode (skip-crawl/skip-ratings/no-publish) then read sixv-series durable `pending 99/100` and returned `quality_status=degraded`, `degraded_sources=[sixv-series]`, `required_degraded_sources=[]`; public current pointer SHA stayed `03e07a...` before/after, proving the observability fix without promotion.
