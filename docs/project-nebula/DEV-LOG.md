...
- Split delivery into authority (`magnetgoogo.com`, GitHub Raw, `api.naoshiquan.com`) and fallback (`cn`, jsDelivr, old workers.dev`) tiers. A second adversarial pass found the authority tier itself still accepted the first HTTP-200 body before decrypt/expiry validation, so all mutable source endpoints are now tried sequentially: each authority must fetch + decrypt + pass freshness before acceptance, and mirrors are reached only after all authorities fail. Dynamic tests cover 5ms stale mirror vs healthy authority and 5ms invalid authority vs 40ms healthy authority.
...
- Last fully device-accepted K30S candidate SHA256 `808061dd940079217898b1269b55a079251a8a7e3343d53ba903974a8999736`: `Inception` completed 67 hosts / 52 pools with 168 high-relevance magnets, 0 skipped, quality hard=0. Post-fix Chinese run completed 52 pools with 120/120 high-relevance and hard=0; old pre-fix report had 120 invalid-surrogate findings. The stricter sequential-authority build SHA256 `4d3ea8c91cf7850f10023250b0a6ed6b75f501b2df7cdff4fcbf5b234198481a` passes TypeScript, 56/56 and Android build, but the execution tool safety layer blocked `adb install` before device execution, so that exact SHA is not marked K30S-accepted.
...
- Pushed App/Gateway commits `fe82b83` (`fix(app): harden media refresh fallback`), `2718d5b` (`fix(app): fail closed on stale sources and bad search metadata`) and `68847dd` (`fix(app): validate source authorities sequentially`) without force push.
- mg-data has real `source-envelope-bot` renewals through 2026-08-13 and production hourly source-sync remains healthy. A final unattended-risk review widened the production workflow refresh threshold from <=24h to <=32h (72h validity unchanged), commit `f9c5105`, increasing the normal 8-hour-schedule failure budget from roughly 16–24h to 24–32h; refresh tests remain 5/5. This session still ended before the next ~2026-08-16 00:17 renewal window, so that particular cycle is not falsely claimed as observed.
- Alert hooks/state machines remain healthy. Shape-only production inspection found URL/security fields present, so transport was temporarily enabled and a strict systemd EnvironmentFile test reached CloudMonitor code; it failed before HTTP because the parsed URL was the literal placeholder `echo`. The URL field is non-HTTPS length 6 and security field length 2, consistent with quoted placeholders rather than a usable endpoint. Transport was restored to disabled; real inbox acceptance remains pending. Failed harness/provider-config attempts were preserved under `_failures`.
---
...
Modules: docs/project-nebula/{DEV-LOG.md,_progress.txt,TEST-RESULT-20260805-v0.2.5全链路公开发布与0.2.3公网升级验收.md}

### Correction and final verification
- Independently re-downloaded the complete R2 APK: 38,510,706 bytes, SHA-256 `642447c18e12f81b167f5a9b711726a6ced28079d7f078678151d05bdea9da70`, matching the local archive, GitHub asset and both Aliyun files.
- Reran update-download contract PASS, release-build contract PASS (`0.2.5/code9`, 147 green, 51 pools), enum validation `357 / ALL VALID`, and APK structure/signature verification PASS with the same certificate as formal v0.2.3.
- K30S remains formal `0.2.5/code9`, firstInstallTime remains `2026-07-28 21:17:01`, no search service residue was found, and Crash/ANR checks returned no package records.
- No production channel, config, website, source inventory or media revision was changed during this correction.
---
...
- R2 was independently re-downloaded in full: 38,510,706 bytes with SHA-256 `642447c18e12f81b167f5a9b711726a6ced28079d7f078678151d05bdea9da70`, exactly matching the local archive, GitHub asset and both Aliyun files. An earlier batched four-channel request hit a DevSpace connector 502 and is retained only as tooling evidence.
...
- K30S retained-data downgrade to formal v0.2.2 displayed the complete password notice, including `蓝奏云下载密码：8888。`; tapping Backup link 1 opened `https://wwbdy.lanzn.com/iDcyE3zn4rcf`.
- Restored K30S to formal v0.2.3/code7 after verification; Fatal, ANR and media-cache commit errors remained zero. The hotfix is remote-config-only and required no APK rebuild.
---

---
Date/Time: 2026-07-31 21:25 (UTC+8)
Version: v0.2.3-lanzou-password-notice-hotfix
Scope: Add the missing Lanzou download password to the live v0.2.2 migration notice without changing the APK, version or download routes
Modules: mg-data/config.json, magnetgoogo-site/config.json, docs/project-nebula/{_progress.txt,TEST-RESULT-20260731-v0.2.2蓝奏云密码提示热修复.md,DEV-LOG.md}

### Notice correction
- Split the migration warning into clear lines and added the standalone text `蓝奏云下载密码：8888。` so formal v0.2.2 users can complete the browser-based migration.
- Added equivalent password text to all ten announcement languages; primary and mirror order remain R2 → Lanzou → GitHub.
- Rebased safely over the concurrent encrypted-source refresh and pushed mg-data commit `9edb4b7`.

### Deployment and verification
- Deployed Cloudflare Pages, synchronized Aliyun config and purged jsDelivr `@main` plus immutable `9edb4b7` paths.
- Cloudflare Pages, GitHub Raw, both gateways and both jsDelivr paths return v0.2.3 with password `8888`; Aliyun server file also parses the password line correctly.
- K30S was not connected in this session, so no new device screenshot was captured. The APK and all download URLs remain unchanged.
---
