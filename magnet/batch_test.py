#!/usr/bin/env python3
"""
Batch test all 697 candidates. Fast probe: only HTTP GET, 8s timeout.
Output to both console and run.log for real-time IDE viewing.
"""
...
log.info(f"=== BATCH TEST: {total} candidates, {len(existing)} existing ===")

for domain, info in sorted(ALL.items()):
    url = info['url'].rstrip('/')
    ...
    if not site_ok:
        log.info(f"[{tested}] {domain:35s} FAIL (unreachable/timeout/403)")
        continue
...
log.info(f"")
log.info(f"{'='*60}")
log.info(f"RESULTS: tested={tested} skipped={skipped} reachable={reachable} WORKING={len(working)} time={elapsed:.0f}s")
for w in working:
