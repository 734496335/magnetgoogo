...
def test_cf_strong_marker_in_body_is_challenge():
    """Body containing a strong marker should be detected regardless of casing."""
    cloak = Tier1Cloak.__new__(Tier1Cloak)
    html = "<html><head></head><body>" + "x" * 200 + "just a MOMENT</body></html>"
    head = html[:8000].casefold()
    challenge = any(marker.casefold() in head for marker in CF_STRONG_MARKERS)
...
def test_weak_marker_in_title_is_challenge():
    """A weak marker in title should be detected through the shared helper."""
    cloak = Tier1Cloak.__new__(Tier1Cloak)
